...
export default function DateSelect({ values, value, setValue, title }: Props) {
  return (
    <Listbox value={value} onChange={setValue}>
      <div className="relative mt-1">
        <Listbox.Button className="relative w-full h-9 cursor-default rounded-lg bg-white py-2 pl-3 pr-10 text-left shadow-md focus:outline-none focus-visible:border-blue-500 focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75 focus-visible:ring-offset-2 focus-visible:ring-offset-blue-300 sm:text-sm">
          <span className="block truncate">{value ? value : title}</span>
                ...
                >
                  {({ selected }) => (
                    <>
                      ...
                      {selected ? (
                        <span className="absolute inset-y-0 left-0 flex items-center pl-1 md:pl-3 text-blue-600">
